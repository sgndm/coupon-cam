...
class ExtraFunctions {
    ...
    public static function WinnerLog($content) {

        $logFile = fopen("log_files/select_winner.txt", "a") or die("Unable to open file!");
        fwrite($logFile, $content);
        fclose($logFile);
    ...
    public static function PushLog($content) {
        ...
        $logFile = fopen("log_files/push_notification_log.txt", "a") or die("Unable to open file!");
        fwrite($logFile, $content);
        fclose($logFile);
                ...
                if($rem_time2 <= 360 ) {
                    ...
                    if($dist <= 0.9) {
                        ...
                        if($red_friday_savings == 0) {
                            ...
                            // check if user has selected before
                            $check_old_winner = OldWinner::where(['device_id' => $user->device_id])->count();

                            if($check_old_winner == 0) {
