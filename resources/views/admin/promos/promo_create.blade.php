...
@section('content')
    <form role="form" method="POST" action="{{ url('/admin/promos/save') }}">
        {{ csrf_field() }}
        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-6">
                <div class="form-group col-sm-12 {{ $errors->has('company_name') ? ' has-error' : '' }}">
                    ...
                    <select class="form-control" id="repeate_promo" onchange="repeatproms()" name="repeate_promo">
                        ...
                        @foreach($promos as $key => $promo)
                            <option value="{{ $promo->promo_id }}" @if(old('repeate_promo') == $promo->promo_id) selected="selected" @endif >{{ $promo->promo_name }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('repeate_promo'))
                        <span class="help-block"><strong>{{ $errors->first('repeate_promo') }}</strong></span>
                    @endif
                </div>
                ...
                <div class="form-group col-sm-12 {{ $errors->has('promo_desc') ? ' has-error' : '' }}">
                    ...
                    <input type="text" class="form-control" id="promo_desc" name="promo_desc" value="{{ old('promo_desc') }}" placeholder="Describe Promo">
                    @if ($errors->has('promo_desc'))
                        <span class="help-block"><strong>{{ $errors->first('promo_desc') }}</strong></span>
                    @endif
                </div>
                <div class="form-group col-sm-12 {{ $errors->has('promo_start') ? ' has-error' : '' }}">
                    <label for="promo_start">Start Time <small class="info-box-title">( in hours & mins )</small></label>
                    <input type="text" class="form-control time-picker" id="promo_start" name="promo_start" value="{{ old('promo_start') }}">
                    @if ($errors->has('promo_start'))
                        <span class="help-block"><strong>{{ $errors->first('promo_start') }}</strong></span>
                    @endif
                </div>
                <div class="form-group col-sm-12 {{ $errors->has('promo_lenght') ? ' has-error' : '' }}">
                    ...
                    <select class="form-control" id="promo_lenght" onchange="AdvanceWarning()" name="promo_lenght">
                        @for($n = 1; $n <= 24; $n++)
                            <option value="{{ $n }}" @if(old('promo_lenght') == $n) selected="selected" @endif> {{ $n }}</option>
                        @endfor
                    </select>
                ...
                <div class="form-group col-sm-12 {{ $errors->has('advance_warning') ? ' has-error' : '' }}">
...
@section('custom_js')
    ...
    <script type="text/javascript">
        ...
        function repeatproms(){
            ...
            window.location = "{{ url('admin/promos/create') }}/"+parseInt(promopage);
        }

        function selectstore(){
            var user = $('select[name="company_name"]').val();
            $.get("{{ url('admin/promos') }}/"+parseInt(user),function(data){
