...
<span class="pull-right" style="display: inline; margin-bottom: 5px;">
    ...
    <form role="form" method="POST" action="{{ url('/admin/promos/filter') }}" >
    <a href="{{ url('admin/promos/trash') }}" class="btn btn-default"><i class="fa fa-trash"></i> Trash</a>
    <a href="{{ url('admin/promos/create') }}" class="btn btn-default"><i class="fa fa-plus"></i> New Promo</a>
    <!-- {{ csrf_field() }}
...
<table id="example" class="display table table-striped" style="width: 100%; cellspacing: 0;">
    ...
    <tbody>
        ...
        <tr>
            ...
            <th>
                <a href="{{ url('admin/promos/edit/'.$promo->promo_id) }}" class="btn btn-xs btn-primary"><i class="fa fa-pencil-square"></i></a>
                <a href="{{ url('admin/promos/delete/'.$promo->promo_id) }}" onclick="return confirm('Are you sure want to delete ?')?true:false" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
            </th>
        </tr>
        @endforeach
    </tbody>
