...
<form role="form" method="POST" action="{{ url('/admin/promos/save') }}">
    {{ csrf_field() }}
    <div class="form-group col-sm-6 {{ $errors->has('repeate_promo') ? ' has-error' : '' }}">
        ...
        <select class="form-control" id="repeate_promo" onchange="repeatproms()" name="repeate_promo">
            ...
            @foreach($promos as $key => $xpromo)
                <option value="{{ $xpromo->promo_id }}" @if($promo->promo_id == $xpromo->promo_id) selected="selected" @endif >{{ $xpromo->promo_name }}</option>
            @endforeach
        </select>
    </div>
...
<script type="text/javascript">
    ...
    function repeatproms(){
        ...
        if(promopage == '0'){
            window.location = "{{ url('admin/promos/create') }}";
        }else{
